...
const Header: FC<HeaderProps> = ({ friend, onShowFriendWallet }) => {
  ...
  if (friend === undefined) {
    ...
    return (
      <Button
        ...
        onClick={() => {
          onShowFriendWallet(friend.publicKey);
        }}
        ...
        <div className="flex px-4 gap-2 justify-start items-center">
          <AtTheRate focused={focused} />
          <Text
            className={
              'max-w-[7rem] font-bold' + ' ' + (focused ? 'text-[#00D99C]' : 'text-white')
            }>
            {friend.name}
          </Text>
